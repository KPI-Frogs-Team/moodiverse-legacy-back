...
from .functions import token_required, get_mood_id, get_user_id, create_mood_record, create_response, get_record_from_db, update_record_in_db, check_date
...
def update_record(decoded_token):
    ...
    new_mood_name = request.json.get('newEmoji')
    ...
    try:
        ...
        return jsonify({'error': 'Error parsing the date.'}), 500
    if not new_mood_name or not new_text or not date:
        return jsonify({'error': 'newEmoji, newText and date fields are required.'}), 400

    new_mood_id = get_mood_id(new_mood_name)
    if not new_mood_id:
        return jsonify({'error': 'Mood not found.'}), 404
    user_id = get_user_id(username)
    if not user_id:
